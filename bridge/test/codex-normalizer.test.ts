...
test("审批：请求带真实可选裁决，裁决回送后 settled 报出选项", () => {
  ...
  const optionIds = approval.options.map((option) => option.id);
  ...
  assert.deepEqual(
    approval.options.map((option) => option.kind),
    ["allow", "allowAlways", "abort", "deny"],
  );
...
test("turn/completed 带上最近一次 token 用量", () => {
  const events = replay(loadFixture());
