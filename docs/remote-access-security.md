...
本文描述手机 ↔ Mac 之间远程通道的安全设计。实现在 `bridge/src/secure/`（TS）与 iOS 侧的同构镜像，跨语言一致性由 `protocol/fixtures/e2ee-handshake.json` 锁死。

对固定输入，两侧的 **transcript 拼接、X25519 共享秘密、HKDF 派生密钥、AES-256-GCM 信封** 逐字节相同。**Ed25519 签名是验签兼容、不是字节一致**：CryptoKit 的 Ed25519 签名带随机化（RFC 8032 允许），同一私钥对同一消息可产出不同签名字节；fixture 存的是 Node 侧确定性签名，Swift 侧断言「能验过 + 篡改即失败 + 自产签名也能被同一公钥验过」，而不是逐字节比对签名字段。
...
| 明文 `/events` `/command` `/git` | 本机调试与（显式放开后的）可信局域网便利通道 | **不是** |
...
**明文不是信任边界。** 监听非回环（`--host 0.0.0.0` 或局域网地址）时，bridge 默认拒绝明文 `/events` `/command` `/git`，手机应走 `/e2ee/*` 或 relay。仅在你完全信任的局域网、且知情承担风险时，才用 `--allow-plaintext-lan` 显式放开。公共 Wi-Fi **必须** 用 relay + E2EE，不要开明文 LAN。
